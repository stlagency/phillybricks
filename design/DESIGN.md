...
- **True Phillies Red `--pb-red`** = SIGNAL ONLY, four sanctioned jobs: (1) the distress score + the dominant segment of the decomposable distress bar + the Distress-lens choropleth; (2) the single primary CTA per screen; (3) the active-parcel highlight + corner ticks on the map. Danger pills downshift to brick (restraint pass); the **urgent** pill is the sole true-red pill and **counts against the budget**.
...
- **Buttons:** square. Primary = red fill + white Space Mono + "→" + 8px shadow (the CTA = the budget; the one mono-caps button); secondary = ink fill + bone Satoshi sentence-case, no shadow; tertiary/ghost = transparent + 1px ink hairline, no shadow (restraint pass tiered the secondaries; primary keeps the full device).
- **Pills:** Space Mono 10px bold, 1px gravel hairline, square (restraint pass). Danger = brick edge / bone fill (downshifted); urgent = red fill/white (the sole crisis pill, counts against budget); neutral = ink fill/bone; aged = bone fill + brick text + brick border.
...
- **Lens switcher:** inline segmented control of 4 hard-bordered cells; active = ink fill + bone label + a blue bottom-marker block (a chunk, not a thin stripe; restraint pass moved this off red — red is reserved for the budget); one active so color means one thing.
...
## Design restraint pass (2026-06-20) — "make the bones breathe"
The brutalist system above stays the identity; this pass dials back *intensity* so hierarchy reads. Three levers, applied at the design-system layer (tokens + shared components) so they cascade:
1. **Label re-cast.** Uppercase + Space Mono was the *default* label treatment (a wall of caps). Now: **UI labels = sentence-case Satoshi** (card/rail/trend heads, eyebrows, filter labels, crumbs, legend/timestrip/community heads, lens-switch tabs, nav links, leads table headers + row-action buttons). **Space Mono stays for all DATA** — values, source stamps, IDs, codes, primary-button text, pills, the distress-bar segments. Card/rail heads keep their 3px ink underline (structure), just lose the caps + tracking.
2. **Tiered device.** The 3–4px ink border + offset hard-shadow no longer sits on *everything*. **Secondaries → 1px gravel hairline, no shadow** (`--pb-border-w-tier1` / `--pb-shadow-tier1`): plain `.pb-cardbox`, ghost buttons, pills, and the scan chrome (lens switcher, legend, timestrip, trend card, community rail). **Heroes keep the full device**: navy header + 4px border, map frame, context rail, distress hero blocks, the primary CTA, and any `.pb-cardbox--frame`/`--mass` (these re-assert the ink edge over the tier-1 base).
