...
- **leads + a mini-CRM**, saved areas, alerts (nightly email digest), and BYO-key skip-trace orchestration — gated behind a low flat subscription. **No data is paywalled — only personalization/automation.**

No black boxes: no ML AVM, every derived number decomposes to its public record.

## Why it exists

Off-market and distressed sales never hit the MLS, and MLS comps are gated to licensed agents. The public record is open. PhillyBricks turns it into an instrument anyone can read.
