/**
 * GET /api/parcel/:pk — the deep-dive bundle (PRD §6, §7.2). Assembles, from the
 * canonical + derived tables: parcel core, assessment-vs-sale, full transfer history
 * (arms-length/estate flagged), open L&I, tax status, nearby crime/311 (via stamped
 * geo ids), comps + value estimate (core selectComps), and the distress decomposition
 * (distress_signal raw → scoreDistress). Returns the frozen ParcelDeepDive.
 */
...
export async function GET(
  ...
  const prow = await sql<(Record<string, unknown> & { parcel_pk: string })[]>`
    ...
    from public.parcel p where p.parcel_pk = ${pk}`;
  if (prow.length === 0) return NextResponse.json({ error: 'parcel not found' }, { status: 404 });
  const p = prow[0]!;
  ...
  const parcel: ParcelCore = {
    ...
    sale_date: (p.sale_date as string | null) ?? null,
  ...
  const [transfersRaw, liRaw, taxDel, taxBal, distressRow, nearbyCrime, nearby311, comps] =
    await Promise.all([
      ...
      sql<Record<string, unknown>[]>`
        (select 'permit' as kind, permit_type as type_code, status, to_char(permit_issued_date,'YYYY-MM-DD') as date from public.permit where parcel_pk = ${pk} order by permit_issued_date desc nulls last limit 25)
        union all
        (select 'violation', violation_type, status, to_char(violation_date,'YYYY-MM-DD') from public.violation where parcel_pk = ${pk} order by violation_date desc nulls last limit 25)
        union all
        (select 'complaint', complaint_type, status, to_char(complaint_date,'YYYY-MM-DD') from public.complaint where parcel_pk = ${pk} order by complaint_date desc nulls last limit 25)
        union all
        (select 'case_investigation', case_type, status, to_char(investigation_date,'YYYY-MM-DD') from public.case_investigation where parcel_pk = ${pk} order by investigation_date desc nulls last limit 25)`,
      sql<Record<string, unknown>[]>`
  ...
  const stamp = (s: string): { source_stamp: string; source_url: string; refreshed_at: string | null } => ({
    source_stamp: s,
    source_url: '',
    refreshed_at: null,
  });
  const tax: TaxStatus = {
    billed: { value: null, ...stamp('OPA') },
    status: delDue && delDue > 0 ? 'delinquent' : del ? 'current' : 'unknown',
    balance_with_penalty: { value: delDue ?? num(taxBal[0]?.total ?? null), ...stamp('Revenue · delinquency') },
  ...
  const nearby: NearbyCounts = {
    crime: {
      ...
      count: nearbyCrime.reduce((s, r) => s + Number(r.n), 0),
      trend: trendOf(nearbyCrime),
    },
    service_requests: {
      window_label: 'last 12 months (neighborhood)',
      count: nearby311.reduce((s, r) => s + Number(r.n), 0),
  ...
  const assessment_vs_sale: ParcelDeepDive['assessment_vs_sale'] = {
    market_value: { value: mv, ...stamp('OPA assessment') },
    last_sale: { value: lastSale, ...stamp(`OPA last sale · ${parcel.sale_date ?? ''}`) },
    assessed_psf: { value: mv && la && la > 0 ? Math.round((mv / la) * 100) / 100 : null, ...stamp('OPA') },
  ...
  // comps is non-null: the parcel exists (we 404'd above) and computeComps re-queries it.
  if (comps === null) return NextResponse.json({ error: 'parcel not found' }, { status: 404 });

  const body: ParcelDeepDive = {
    parcel,
    assessment_vs_sale,
    transfers,
    li,
