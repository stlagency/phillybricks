...
| **M4** | Serving + map. **DEPLOYED LIVE** → https://phillybricks.vercel.app. All 5 read APIs + `/api/geo` serve real M3 data; MapLibre 4-lens scan + per-parcel PMTiles on Supabase Storage; nightly tile rebuild automated (CI secrets set) | ✅ **done** |
...
| **M7** | Accounts: Supabase Auth → the `lib/auth.ts` seam; Stripe sub + **verified webhook** + entitlements; saved areas (3 modes); alert diff → `alert_event` → **nightly Resend digest** + in-app feed | ⏳ |
