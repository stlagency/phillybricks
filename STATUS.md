...
Ingestion-first per PRD §9. State history (change-logs) only accrues forward and is the one irrecoverable asset. **→ Resume point for the next session: [`docs/NEXT_SESSION.md`](docs/NEXT_SESSION.md) — finish M4: wire the MapLibre map to the (done, live-verified) read APIs + deploy. Needs Vercel Pro + R2 keys.**
...
| **M4** | Serving + map. **Read APIs DONE + live-verified** against real M3 data: `/api/scan` (4 lenses, quantile buckets, period ranges), `/api/parcel/:pk` (full deep-dive bundle), `/api/comps` (deterministic ladder + transparent estimate), `/api/leads` (scored/filtered/paginated), `/api/boundaries` (GeoJSON). `next build` clean. **Remaining (needs Vercel Pro + R2):** deploy + MapLibre map wiring + nightly PMTiles→R2 | 🔄 **APIs done; map+deploy need creds** |
